...
def te(data, processed_file):
    ...
    for item in tqdm(data.itertuples(index=True)):  # 遍历文件中的每个条目
        # 提取文本内容
        text = text_filter_english(item.text) # 获取条目中的文本内容
        item_id = item.id
        label = item.label
        # 调用TexSmart工具进行实体识别
...
if __name__ == '__main__':
    data_dir = '../data/twitter_cheng'
    processed_dir = '../data/twitter_cheng/processed'
    data_type = ['test'] # 'train','valid',
    for t in data_type:
        data = pd.read_csv('{}/{}.csv'.format(data_dir,t),encoding='utf-8')
