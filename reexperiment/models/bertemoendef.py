...
class BERTEmo_ENDEFModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        emotion_feature = torch.zeros(feature.size(0), 47).cuda()

        feature = torch.cat([feature, emotion_feature], dim=1)
    ...
    def train(self, logger = None):
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                # loss = loss_fn(pred, label.float().unsqueeze(1)) + 0.2 * loss_fn(entity_pred,
