...
class BERTEmoModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # print(f'bert_feature shape: {bert_feature.shape}')


        feature, _ = self.rnn(bert_feature)

        # print(f'rnn feature shape: {feature.shape}')
        feature, _ = self.attention(feature, masks)
        # -
        # print(f'attention feature shape: {feature.shape}')
        # print(f'emotion shape: {emotion.shape}')
        # 拼接
        # concatenated = torch.cat([feature, emotion], dim=1)
